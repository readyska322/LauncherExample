...
private List<WifiActivity.device> wifiList;
private Context context;
private View.OnClickListener mOnClickListener;

    public WifiScanAdapter(List<WifiActivity.device> wifiList, Context context) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
           ...
           WifiActivity.device device=wifiList.get(position);
           String name=device.getName().toString();

             ((MyViewHolder) holder).vName.setText(name);
             ((MyViewHolder) holder).vName.setTag(device);

            ((MyViewHolder) holder).vImage.setImageResource(R.drawable.ic_wifi_black_24dp);
